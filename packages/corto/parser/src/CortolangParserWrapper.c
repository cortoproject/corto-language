...
void parser_DisplayRecognitionError(ANTLR3_BASE_RECOGNIZER_struct* recognizer, uint8_t** tokens)
{
     ...
     * displayRecognitionError (virtual method)
     */
    displayRecognitionError(recognizer, tokens);
    pANTLR3_PARSER parser = (pANTLR3_PARSER)(recognizer->super);
    pCortolangParser ctx = (pCortolangParser)parser->super;
    parser_BaseParser cortoParser = ctx->parser_data->parser;
    parser_BaseParser_displayRecognitionError(cortoParser, (corto_word)recognizer, (corto_word)tokens);
}

parser_data* parser_parserDataNew(parser_BaseParser parser)
{
    parser_data* data = new parser_data;
    if (!data) {
        goto errorDataAlloc;
    }
    corto_claim(parser);
