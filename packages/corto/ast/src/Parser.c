...
ast_Expression ast_Parser_createBinaryTernary(ast_Parser this, ast_Expression lvalue, ast_Expression rvalue, corto_operatorKind operator) {
    ...
    case CORTO_ASSIGN_AND:
        ...
        corto_ptr_setref(&result, rvalue);
        break;
    default:
...
ast_Expression ast_Parser_delegateAssignment(ast_Parser this, ast_Expression lvalue, ast_Expression rvalue) {
    ...
    corto_ptr_setref(&variables[0].object, lvalue);
...
ast_Expression ast_Parser_explodeComma(ast_Parser this, ast_Expression lvalues, ast_Expression rvalues, ast_ExpandAction action, void *userData) {
    ...
    lvalueCount = corto_ll_size(lvalueList);
    rvalueCount = corto_ll_size(rvalueList);
...
ast_Expression ast_Parser_expandComma(ast_Parser this, ast_Expression lvalues, ast_Expression rvalues, ast_ExpandAction action, ast_ExpandAction combine, void *userData) {
    ...
    if ((lvalueCount > 1) && (rvalueCount == 1)) {
        ...
        ast_Parser_error(this, "mismatch in expression count of binary operation (left = %d, right = %d)",
                          corto_ll_size(lvalueList), corto_ll_size(rvalueList));
        goto error;
    }
...
ast_Storage ast_Parser_observerCreate(ast_Parser this, corto_string id, ast_Expression object, corto_eventMask mask, ast_Object dispatcherVar) {
    ...
    if (!id) {
        ...
        corto_ptr_setref(&observer->observable, observable);
        observer->mask = mask;
        corto_ptr_setref(&observer->dispatcher, dispatcher);
    } else {
...
ast_Expression ast_Parser_parseIntern(ast_Parser parser, corto_object scope, corto_int32 *err) {
    ...
    corto_ptr_setref(&parser->scope, scope);
    ...
    if (parser->block) {
        if (corto_ll_size(parser->block->statements)) {
...
corto_string _ast_Parser_argumentToString(
    ast_Parser this,
    corto_type type,
    corto_string id,
    bool reference)
...
ast_Node _ast_Parser_binaryExpr(
    ast_Parser this,
...
/* $begin(corto/ast/Parser/bind) */
    ...
    if (this->pass && function) {
        if (function->kind == Ast_ObjectStorage) {
            ...
            corto_ptr_setref(&binding->function, ast_Object(function)->value);
            corto_ptr_setref(&binding->impl, block);
...
int16_t _ast_Parser_bindOneliner(
    ast_Parser this,
    ast_Storage function,
    ast_Block block,
    ast_Expression expr)
...
/* $begin(corto/ast/Parser/blockPop) */
    ...
    if (this->pass) {
        if (this->block) {
            corto_ptr_setref(&this->block, this->block->parent);
        } else {
...
ast_Block _ast_Parser_blockPush(
    ast_Parser this,
    bool presetBlock)
{
/* $begin(corto/ast/Parser/blockPush) */
    ...
    if (this->pass) {
        ...
        if (!this->blockPreset || presetBlock) {
            ...
            corto_ptr_setref(&this->block, newBlock);
        }
...
ast_Expression _ast_Parser_callExpr(
    ast_Parser this,
...
ast_Expression _ast_Parser_castExpr(
    ast_Parser this,
    corto_type lvalue,
    ...
    if (this->pass) {
        ...
        if (lvalue == rvalueType) {
            ...
            if (rvalueType) {
                ...
                corto_ptr_setref(&rvalue->type, lvalue);
                castRequired = FALSE;
            }
        }
...
ast_Storage _ast_Parser_declaration(
    ast_Parser this,
    corto_type type,
    corto_string id,
    bool isReference)
...
ast_Storage _ast_Parser_declareFunction(
    ast_Parser this,
...
ast_Block _ast_Parser_declareFunctionParams(
    ast_Parser this,
    ast_Storage function)
{
...
int16_t _ast_Parser_defineScope(
    ast_Parser this)
{
...
int16_t _ast_Parser_defineVariable(
    ast_Parser this,
    ast_Storage object)
{
...
ast_Expression _ast_Parser_elementExpr(
    ast_Parser this,
...
corto_type _ast_Parser_getComplexType(
    ast_Parser this)
{
...
ast_Expression _ast_Parser_getLvalue(
    ast_Parser this,
    bool assignment)
{
...
corto_type _ast_Parser_getLvalueType(
    ast_Parser this,
    bool assignment)
{
...
ast_Node _ast_Parser_ifStatement(
    ast_Parser this,
...
int16_t _ast_Parser_initDeclare(
    ast_Parser this,
    ast_Expression ids)
{
/* $begin(corto/ast/Parser/initDeclare) */
    ...
    while (corto_iter_hasNext(&it)) {
        ...
        if (ast_Node(s)->kind == Ast_StorageExpr) {
            ...
            if (scope) {
                corto_ptr_setref(&oldScope, this->scope);
                corto_ptr_setref(&this->scope, scope);
            }
            ...
            if (scope) {
                corto_ptr_setref(&this->scope, oldScope);
                corto_ptr_setref(&oldScope, NULL);
            }
        } else {
            ast_Parser_error(this, "invalid usage of comma expression");
            goto error;
...
int16_t _ast_Parser_initKeyValuePop(
    ast_Parser this)
{
...
int16_t _ast_Parser_initKeyValuePush(
    ast_Parser this)
{
...
int16_t _ast_Parser_initKeyValueSet(
    ast_Parser this,
    ast_Expression expr)
{
...
int16_t _ast_Parser_initMember(
    ast_Parser this,
    corto_string member)
{
...
ast_Expression _ast_Parser_initPushExpression(
    ast_Parser this)
{
/* $begin(corto/ast/Parser/initPushExpression) */
    ...
    if (this->pass) {
        ...
        corto_ptr_setref(&this->initializers[this->initializerCount], initializer);
...
ast_Expression _ast_Parser_initPushIdentifier(
    ast_Parser this,
    ast_Expression type)
{
/* $begin(corto/ast/Parser/initPushIdentifier) */
    ...
    if ((!this->pass && !isDynamic) || forceStatic) {
        corto_object o;
        o = corto_declare(t);
        corto_ptr_setref(&variables[0].object, ast_Expression(ast_ObjectCreate(o)));
        ...
        corto_ptr_setref(&variables[0].object, var);
...
int16_t _ast_Parser_initPushStatic(
    ast_Parser this)
{
/* $begin(corto/ast/Parser/initPushStatic) */
    ...
    for(i=0; i<this->variableCount; i++) {
        corto_ptr_setref(&variables[i].object, ast_Expression(this->variables[i]));
        variables[i].key = 0;
...
ast_Expression _ast_Parser_lookup(
    ast_Parser this,
    corto_string id)
{
...
ast_Expression _ast_Parser_memberExpr(
    ast_Parser this,
    ast_Expression lvalue,
    ast_Expression rvalue)
{
...
ast_Storage _ast_Parser_observerDeclaration(
    ast_Parser this,
    corto_string id,
...
ast_Expression _ast_Parser_parseExpression(
    ast_Parser this,
    corto_string expr,
    ast_Block block,
    corto_object scope,
...
/* $begin(corto/ast/Parser/parseExpression) */
    ...
    corto_ptr_setref(&this->block, block);
    corto_ptr_setref(&this->scope, scope);
    ...
    if (block) {
        ...
        if (block->statements) {
            if (corto_ll_size(block->statements) == 1) {
                ast_Node node;
...
/* $end */
int16_t _ast_Parser_parseFunction(
    corto_function f,
    corto_string expr,
    bool inverse)
{
/* $begin(corto/ast/Parser/parseFunction) */
    ...
    corto_ptr_setref(&parser->block, block);
    ...
    if (!resultLocal && result && result->type) {
        ...
        corto_ptr_setref(&f->returnType, returnType);
    ...
    if (parser->bindings && corto_ll_size(parser->bindings)) {
...
corto_type _ast_Parser_parseType(
    corto_string expr,
    corto_object scope)
{
...
void _ast_Parser_popComplexType(
    ast_Parser this)
{
...
/* $begin(corto/ast/Parser/popScope) */
    ...
    /* Restore scope */
    corto_ptr_setref(&this->scope, previous);
/* $end */
}

ast_Expression _ast_Parser_postfixExpr(
    ast_Parser this,
    ast_Expression lvalue,
    corto_operatorKind _operator)
{
...
void _ast_Parser_pushComplexType(
    ast_Parser this,
    ast_Expression lvalue)
{
/* $begin(corto/ast/Parser/pushComplexType) */
    ...
    if (lvalue) {
        corto_ptr_setref(&this->complexType[this->complexTypeSp], ast_Expression_getType(lvalue));
    } else {
        corto_ptr_setref(&this->complexType[this->complexTypeSp], NULL);
    }
    this->complexTypeSp++;
...
/* $begin(corto/ast/Parser/pushLvalue) */
    ...
    corto_ptr_setref(&this->lvalue[this->lvalueSp].expr, lvalue);
...
int16_t _ast_Parser_pushPackage(
    ast_Parser this,
    corto_string name)
{
/* $begin(corto/ast/Parser/pushPackage) */
    ...
    corto_ptr_setref(&this->scope, root_o);
...
void _ast_Parser_pushReturnAsLvalue(
    ast_Parser this,
    corto_function function)
{
...
corto_object _ast_Parser_pushScope(
    ast_Parser this)
{
/* $begin(corto/ast/Parser/pushScope) */
    ...
    if (this->variables[0]) {
        corto_ptr_setref(&this->scope, ast_Object(this->variables[0])->value);
        ast_Parser_reset(this);
    } else {
        goto error;
    }
...
ast_Expression _ast_Parser_ternaryExpr(
    ast_Parser this,
    ast_Expression cond,
...
ast_Expression _ast_Parser_unaryExpr(
    ast_Parser this,
    ast_Expression lvalue,
    corto_operatorKind _operator)
{
...
ast_Node _ast_Parser_updateStatement(
    ast_Parser this,
    ast_Expression expr,
    ast_Block block)
{
...
/* $begin(corto/ast/Parser/whileStatement) */
    ...
    if (this->pass) {
        ...
        if (isUntil) {
            corto_ptr_setref(&this->block->_while, result);
        }
    }
