...
int16_t ast_InitializerExpression_defineObject(
    ast_InitializerExpression this)
{
    ast_InitOper *elem = ast_InitOperList__append_alloc(this->operations);
    ...
    ast_Expression variable)
{
    ...
    if (ast_Storage(var)->kind == Ast_TemporaryStorage) {
        ast_Init init = ast_Init__create(NULL, NULL, ast_Storage(var));
    ...
    initializer = ast_DynamicInitializer__create(NULL, NULL, ast_Initializer(this)->variables, 1, this->assignValue);
    /* Walk operations */
