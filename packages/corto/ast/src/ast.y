...
/* ======================================================================== */
literal_expr
    : BOOLEAN               {$$=ast_Boolean__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | CHARACTER             {$$=ast_Character__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | INTEGER               {$$=ast_Integer__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | SIGNEDINTEGER         {$$=ast_SignedInteger__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | FLOATINGPOINT         {$$=ast_FloatingPoint__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | STRING                {$$=ast_String__create(NULL, NULL, $1); ast_Parser_collect(yparser(), $$);}
    | NUL                   {$$=ast_Null__create(NULL, NULL); ast_Parser_collect(yparser(), $$);}
    ;
...
postfix_expr
    ...
    | postfix_expr '.' any_id                     {ast_String str = ast_String__create(NULL, NULL, $3); if (!str) {YYERROR;} $$ = ast_Parser_memberExpr(yparser(), $1, ast_Expression(str)); corto_release(str); fast_op;}
    ...
    | comma_expr ',' conditional_expr {
        if ($1 && $3) {
            if (ast_Node($1)->kind != Ast_CommaExpr) {
                $$ = ast_Comma__create(NULL, NULL); fast_op;
